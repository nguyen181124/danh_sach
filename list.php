      ...
      <table class="mx-auto mt-7" style="width:70%">
        ...
        <?php for ($i = 0; $i < count($listuser); $i++) { ?>
          <tr>
            ...
            <td id="test" class="text-center"><input class="check" value="<?php echo $i; ?>" type="checkbox"></td>
          </tr>
        <?php } ?>
      </table>
    </div>
    <form class="table" action="" method="post">
      <div class="mt-12 ">
        <div class="flex flex-col">
          Họ và tên<input class="input_infor border-2 " type="text" name="name" disabled value='<?php ?>'>
          <span class="error text-red-600 block">
            <?php echo $ename; ?>
          </span>
          Giới tính <select class="input_infor border-2  rounded-none min-h-7 bg-white" name="gender" disabled>
            <option value="Nam">Nam</option>
            <option value="Nữ">Nữ</option>
            <option value="Khác">Khác</option>
          </select>
          Năm sinh <input class="input_infor border-2 " type="text" name="birth" disabled value='<?php ?>'>
          <span class="error text-red-600 block">
            <?php echo $ebirth; ?>
          </span>
          Điểm <input class="input_infor border-2 " type="text" name="score" disabled value='<?php ?>'>
          <span class="error text-red-600 block">
            <?php echo $escore; ?>
          </span>
        </div>
        <div class="submit flex mt-4 justify-between">
          <a href="/add"><input class="add border-2 border-black p-2 w-20" type="button" name="Check" value="Thêm" /></a>
          <input class="border-2 border-black p-2 w-20" type="button" name="Check" value="Xóa" />
          <a href="/fix"><input class="fix border-2 border-black p-2 w-20" type="button" name="Check" value="Sửa" /></a>
        </div>
      </div>
      <br>
    </form>
